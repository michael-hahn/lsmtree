...
std::pair<unsigned long, std::string> Printer::key_value_pairs(Node *aRoot, int fd, std::set<std::pair<int, bool>, set_compare>& found_once) {
    std::string rtn = "";
    unsigned long counter = 0;
    if (!aRoot) {
        std::pair<unsigned long, std::string> empty_pair(counter, rtn);
        return empty_pair;
    }
    auto node = aRoot;
    while (!node->isLeaf()) {
    ...
    auto leafNode = static_cast<LeafNode*>(node);
    while (leafNode) {
        counter += leafNode->key_value_pairs(fd, found_once).first;
        rtn += leafNode->key_value_pairs(fd, found_once).second;
        leafNode = leafNode->next();
    }
